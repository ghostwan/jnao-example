 ...
 */
public class Scenar1 implements StartInterface {
	...
	public void start(String robotIP) {
		...
		try {
			...
			alMemory.subscribeToEvent("RearTactilTouched", "onTouchEnd::(f)", this);
		...
		try {
			switch (currentStep) {
				...
				case STEP_STARTUP:
					...
					alMemory.subscribeToEvent("MiddleTactilTouched", "onHeadTouch::(f)", this);
					tts.say("hello world!");
					break;
				...
				case STEP_JOKES:
					...
					alSpeechRecognition.subscribe(APP_NAME);
					alMemory.subscribeToEvent("WordRecognized", "onWordRecognizedJokes::(m)", this);
					break;
