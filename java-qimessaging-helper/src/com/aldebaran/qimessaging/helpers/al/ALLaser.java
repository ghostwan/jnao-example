 ...
 */
public class ALLaser extends ALModule {

    public ALLaser(Session session) {
        super(session);
    }
    ...
    public void laserOFF() throws CallError, InterruptedException {
        if (isAsynchronous)
            service.call("laserOFF");
        else
            service.call("laserOFF").get();
    }
    ...
    public void laserON() throws CallError, InterruptedException {
        if (isAsynchronous)
            service.call("laserON");
        else
            service.call("laserON").get();
    }

    /**
     * 115 setOpeningAngle    Void (Float,Float)
     ...
     */
    public void setOpeningAngle(Float param1, Float param2) throws CallError, InterruptedException {
        if (isAsynchronous)
            service.call("setOpeningAngle", param1, param2);
        else
            service.call("setOpeningAngle", param1, param2).get();
    }

    /**
     * 116 setDetectingLength Void (Int32,Int32)
     * Set detection threshold of the laser
     * length_min_l: int containing the min length that the laser will detect(mm), this value must be upper than 20 mm
     * length_max_l: int containing the max length that the laser will detect(mm), this value must be lower than 5600 mm
     * * Signals:
     */
    public void setDetectingLength(Integer param1, Integer param2) throws CallError, InterruptedException {
        if (isAsynchronous)
            service.call("setDetectingLength", param1, param2);
        else
            service.call("setDetectingLength", param1, param2).get();
    }
