...
public class ALStore extends ALModule {
    ...
    public void stopUpdate() throws CallError, InterruptedException {
        if (isAsynchronous)
            service.call("stopUpdate");
        else
            service.call("stopUpdate").get();
    }

    /**
     * 116 status        List<PackageInfoStatus> ()
     * Get package status.
     * * Signals:
     */
    public List<Object> status() throws CallError, InterruptedException {
        return (List<Object>) service.call("status").get();
    }
