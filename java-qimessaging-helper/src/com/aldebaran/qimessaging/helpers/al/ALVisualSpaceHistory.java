 ...
 */
public class ALVisualSpaceHistory extends ALModule {

    public ALVisualSpaceHistory(Session session) {
        super(session);
    }
    ...
    /**
     * 126 setGridPrecision    Void (Float)
     * Sets grid precision
     * precision: float, ratio between camera field of view and grid cell field of view in each dimension. Setting the grid precision will reset the grid.
     */
    public void setGridPrecision(Float param1) throws CallError, InterruptedException {
        if (isAsynchronous)
            service.call("setGridPrecision", param1);
        else
            service.call("setGridPrecision", param1).get();
    }

    /**
     * 127 getGridPrecision    Float ()
     * Gets grid precision
     * return: Grid precision
     */
    public Float getGridPrecision() throws CallError, InterruptedException {
        return (Float) service.call("getGridPrecision").get();
    }
    ...
    public void resetGrid() throws CallError, InterruptedException {
        if (isAsynchronous)
            service.call("resetGrid");
        else
            service.call("resetGrid").get();
    }
